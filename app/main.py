...
DELIVERY_COSTS = {"courier": 300.0, "sdek": 250.0, "ozon": 199.0}
PROMO_CODES = {
    code.strip().upper(): int(discount)
    for item in os.getenv("PROMO_CODES", "").split(",")
    if ":" in item
    for code, discount in [item.split(":", 1)]
    if code.strip() and discount.strip().isdigit() and 0 < int(discount) <= 100
}
...
class OrderCreate(BaseModel):
    ...
    promo_code: Optional[str] = Field("", max_length=32)
...
async def validate_promo(promo: PromoCodeIn):
    discount_percent = PROMO_CODES.get(promo.code.strip().upper())
...
async def create_order(
    ...
    discount_percent = PROMO_CODES.get((order.promo_code or "").strip().upper(), 0)
    total = max(0, subtotal - subtotal * discount_percent / 100) + delivery_cost
    db_order = Order(
    ...
    return {"success": True, "order_id": db_order.id, "total": total, "discount_percent": discount_percent}
