...
SELLER_IDS = os.getenv("SELLER_IDS", "7890854793,940063562").split(",")
SELLER_API_KEY = os.getenv("SELLER_API_KEY", "admin123")
FRONTEND_URL = os.getenv("FRONTEND_URL", "https://amir2013bikecom-crypto.github.io/cosmetics-miniapp-frontend")
...
async def get_orders(buyer_id: str):
    async with async_session() as session:
...
async def admin_orders(x_seller_key: Optional[str] = Header(None)):
    if x_seller_key != SELLER_API_KEY:
        raise HTTPException(status_code=403, detail="Invalid seller key")
    async with async_session() as session:
        result = await session.execute(
...
async def update_status(order_id: int, update: StatusUpdate, x_seller_key: Optional[str] = Header(None)):
    if x_seller_key != SELLER_API_KEY:
        raise HTTPException(status_code=403, detail="Invalid seller key")
    async with async_session() as session:
